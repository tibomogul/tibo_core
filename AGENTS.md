...
**TiboCore** is a Rails 8.1 mountable engine gem that provides core domain logic with the "Solid trifecta" integration:
...
├── lib/
│   ├── tibo_core.rb        # Main entry point, requires dependencies
│   └── tibo_core/
...
└── tibo_core.gemspec       # Gem specification
```

## Key Conventions

### Engine Isolation
- The engine uses `isolate_namespace TiboCore` - all models, controllers, and routes are namespaced under `TiboCore::`
- URL helpers require `TiboCore::Engine.routes.url_helpers`
...
- Include `TiboCore::Engine.routes.url_helpers` for route helpers in specs
